...
for (const token of ["#view", "#yaw", "#pitch", "#distance", "#pose"]) {
  if (!html.includes(token.slice(1)) && !js.includes(token)) {
    throw new Error(`Missing smoke token: ${token}`);
  }
}

for (const token of ["addEventListener(\"input\", render)", "JSON.stringify(pose", "orbitPose("]) {
  if (!js.includes(token)) {
    throw new Error(`Missing interactive pose token: ${token}`);
  }
}

for (const token of ["canvas", "input", "pre"]) {
